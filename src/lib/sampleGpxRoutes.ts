...
const SAMPLE_ROUTES: SampleRouteMeta[] = [
  { file: "Javornik_z_Limbachu.gpx", surface: "paved" },
  { file: "Karpaty (1).gpx", surface: "paved" },
...
export function getSampleRoutesFromGpx(): Route[] {
  const dataDir = path.join(process.cwd(), "sample-data");

  return SAMPLE_ROUTES.map(({ file, surface }) => {
